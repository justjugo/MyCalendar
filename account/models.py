...
class MyAccountManager(BaseUserManager):
    ...
    def create_user(self, email, nick_name, password, profile_img=None):
        ...
        if not nick_name :
            raise ValueError('user should have a nick name')
        user = self.model(email=self.normalize_email(email), nick_name=nick_name, profile_img=profile_img)
        user.set_password(password)
        user.save(using=self._db)
        return user

    def create_superuser(self, email, nick_name, password, profile_img=None):
        user = self.create_user(email=self.normalize_email(email), nick_name=nick_name, profile_img=profile_img, password=password)
        user.is_admin = True
        user.is_staff = True
